...
MatAllocator* Mat::getStdAllocator()
{
    ...
    Mat::Mat(const Mat &m)
        : flags(m.flags), dims(m.dims), rows(m.rows), cols(m.cols), data(m.data),
          ...
          u(m.u), size(&rows), step(0), dpi(m.dpi)
    {
        if (u)
    ...
    void Mat::reserveBuffer(size_t nbytes)
    {
        ...
        int newrows = nelems > size_t(INT_MAX) ? nelems > 0x400 * size_t(INT_MAX) ? nelems > 0x100000 * size_t(INT_MAX) ? nelems > 0x40000000 * size_t(INT_MAX) ? size_t(INT_MAX) : 0x40000000 : 0x100000 : 0x400 : 1;
#else
    ...
    Size getContinuousSize2D(Mat &m1, Mat &m2, int widthScale)
    {
        ...
        if (sz1 != m2.size()) // reshape all matrixes to the same size (#4159)
        {
            ...
            CV_Assert(is_m1_vector);
            CV_Assert(is_m2_vector);
